...
beam.refine <- function(dx, dy, beam.size = 1, depth = 4){
  ...
  # refine(matrix(c(rep(0, 4), rep(1, 4)), ncol = 4, byrow = TRUE), 4)
  ...
  res.box <- list()
  res.tab <- as.data.frame(matrix(ncol = 2, nrow = 0))
  
  for(i in 1:ncol(box.init)){
    tmp <- refine(dx, dy, box.init, i)
    res.box <- c(res.box, list(c(list(tmp[[1]]), i)))
    res.tab <- rbind(res.tab, c(tmp[[2]], tmp[[3]]))
  }
  ...
  if(depth > 1){
    for(j in 1:(depth - 1)){
      ...
      for(k in 1:nrow(res.tab)){
        if(res.tab[k, 2] == 1){
          inds.r <- dims[!(dims %in% res.box[[k]][[2]])]
          for(i in inds.r){
            tmp <- refine(dx, dy, res.box[[k]][[1]], i)
            res.box <- c(res.box, list(c(list(tmp[[1]]), list(c(i, res.box[[k]][[2]])))))
            res.tab <- rbind(res.tab, c(tmp[[2]], tmp[[3]]))
          }
        }
      }
