...
class MungeGithub:
    ...
    def __init__(self, local_var=5, p_swap=0.5):
        ...
        self.fnn = importr('fnn')
        self.munge = importr('munge')

    def fit(self, X: pd.DataFrame, **kwargs):
        self.nn_idx = self.fnn.knn_index(X, 1, "kd_tree")
        self.data = X
        return self
    ...
    def sample(self, size: int):
        if size > self.data.shape[0]:
            ...
            g_data = self.munge.munge(self.data, reps=k, local_var=self.local_var, p_swap=self.p_swap, nn_ind=self.nn_idx)
        else:
            g_data = self.data
...
class Munge:
    ...
    def get_rstring(self):
        return open("src/main/R/Munge.R", mode="r").read()
