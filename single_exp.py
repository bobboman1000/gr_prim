...
for d in datasets:
    for sd in d:
        exp_man.add_experiment(sd, DummyGenerator(), DummyMetaModel(), c.discovery_algs["prim"], name="dummy_dummy_prim_" + sd.name + str(sd.fragment_size), new_samples=2500, fragment_limit=30, enable_probabilities=True, scaling=ZERO_ONE_SCALING)
        exp_man.add_experiment(sd, DummyGenerator(), c.metamodels["classRF"], c.discovery_algs["prim"], name="dummy_dummy_prim_" + str(sd.fragment_size), new_samples=2500, fragment_limit=30, enable_probabilities=True, scaling=ZERO_ONE_SCALING)
        exp_man.add_experiment(sd, kde, c.metamodels["classRF"], c.discovery_algs["prim"], name="kde-si-hard_classRF_prim_" + str(sd.fragment_size), new_samples=2500, fragment_limit=30, enable_probabilities=True, scaling=Z_SCORE_SCALING)
